...
export default function Sell() {
 ...
 const sellTokenv3 = async () => {
    ...
    try {
      ...
      const approveTxLink = await approveToken(tokenAddress, sender, uni_v3_router);
 ...
 const approveToken = async(tokenAddress, sender, uni_v3_router) => {

    // const uni_v3_router = "0xE592427A0AEce92De3Edee1F18E0157C05861564";
    const wethAddress = '0xB4FBF271143F4FBf7B91A5ded31805e42b2208d6';
    const myAddress = sender;

    tokenAddress = provider.utils.toChecksumAddress(tokenAddress);
    const tokenContract = new provider.eth.Contract(contractAbi, tokenAddress);
    ...
    const isApproved = allowance >= tokenSupply;

    if (isApproved) {
        return true;
    }

    const max = await tokenContract.methods.totalSupply().call();
    const nonce = await provider.eth.getTransactionCount(myAddress);
    const gasPrice = await provider.eth.getGasPrice();
    ...
    while (true) {
      try {
        const approveTx = await tokenContract.methods.approve(contractAddress, max).send({
          nonce: nonce,
          from: myAddress,
          gasPrice: gasPrice,
        });
        ...
        if (approveTx.status !== true) {
          console.log('Approval transaction failed. Aborting.');
        }
  
        const txHash = approveTx.transactionHash;
        const approveTxLink = txHash;
        const txResult = `Approved token: ${approveTxLink}`;
        console.log(txResult);
        break;
      } catch (e) {
        if (!error) {
